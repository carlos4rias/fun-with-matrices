...
/**
 *  Simple thread_pool that creates `threadCount` threads upon its creation,
...
class thread_pool {
public:
  explicit thread_pool(const unsigned int threadCount)
   ...
   *  be done. After invoking `thread_pool::JoinAll`, the pool can no
   *  longer be used.
   */
  void JoinAll() {
    // scoped lock
...
#endif // CONCURRENT_thread_pool_H
